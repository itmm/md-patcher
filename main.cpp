...
bool starts_with(const std::string &base, const std::string &prefix) {
...
bool do_wildcard(const std::string &indent, Lines::iterator &cur, const Lines::const_iterator &end) {
	if (! next()) {
		std::cerr << "end of file after wildcard\n";
		return false;
	}
	while (cur != end && (line == "```" || *cur != line) && starts_with(*cur, indent)) { ++cur; }
	return true;
}
...
bool read_patch(Lines &lines) {
	...
	while (line != "```") {
		if (line_is_wildcard(indent)) {
			if (! do_wildcard(indent, cur, lines.end())) { return false; }
			continue;
...
void change_file(std::string &file) {
	...
	if (last_idx != std::string::npos && last_idx > 0) {
		const auto start_idx { line.rfind('`', last_idx - 1) + 1 };
		if (start_idx != std::string::npos && start_idx < last_idx) {
			file = line.substr(start_idx, last_idx - start_idx);
		}
	}
}
